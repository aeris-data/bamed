...
# 
# USER_EMAIL            e-mail address where the job notification will be sent
# 
...
SING_CONTAINER_PATH="/home/resos/BAMED/src/bamed.sif"
PYTHON_PATH="/home/resos/BAMED/src/bamed.py"
...
function help() {
    ...
    echo "# | USER_EMAIL='name1.name2@mail.com'                                              |"
    ...
    echo '# '
    echo '# USER_EMAIL            e-mail address where the job notification will be sent'
    echo '# '
...
function check_args(){
    ...
    if [ -z ${USER_EMAIL} ]; then printf "%s - ERROR : USER_EMAIL variable is either empty or not set at all\n" "$(date +'%d/%m/%Y - %H:%M:%S')"; exit_status=1; fi
...
function main(){
    ...
    DATA_START_DATE=$(date -d "$(date -d "${START_DATE}" +%Y%m%d) - 1 day" +%Y%m%d)
    DATA_END_DATE=$(date -d "$(date -d "${END_DATE}" +%Y%m%d) 1 1 day" +%Y%m%d)
    DATA_N_DAYS=$((${N_DAYS}+2))
    check_the_date

    local LAUNCH_WORKING_DIR="${WORKING_DIR}/${ID_NAME}/${START_DATE}_${END_DATE}"
...
retrieve,
    ...
    grid=${GRID_RES}/${GRID_RES},
    area=${AREA},
    date=${DATA_START_DATE}/to/${DATA_END_AN_DATE},
    time=00/06/12/18,
    param=130/131/132/133/135,
    target="${DATA_DIR}/[date]_[time]_[step].grib"
retrieve,
...
EOF
    ...
    if [ -z ${SERVER_USER} ] || [ -z ${SERVER_ADDRESS} ] || [ -z ${SERVER_DATA_DIR} ]; then
        ...
        ssh "${SERVER_USER}@${SERVER_ADDRESS}" ${cmd}
        ...
        for RESULT_GRIB_FILE in ${DATA_DIR}/*.grib; do
            ...
            for try_ind in {1..10}; do
                scp -o ServerAliveInterval=10 -o ServerAliveCountMax=5 -q "${SRC_PATH}" "${DST_PATH}"
                status=$?
                if (( ${status} == 0 )); then
        ...
        if (( ${NOT_TRANSFERRED_FILES} != 0 )); then 
            ...
            ssh "${SERVER_USER}@${SERVER_ADDRESS}" ${cmd}
            scp -q "${SIMU_CONFIG_FILEPATH}" "${DST_PATH}/"
            scp -q "${REMOTE_JOB_FILEPATH}" "${DST_PATH}/"
            ...
            jobID=$(ssh "${SERVER_USER}@${SERVER_ADDRESS}" ${cmd})
            echo "$(date +'%d/%m/%Y - %H:%M:%S') - ${jobID}"
            jobID="${jobID//[!0-9]/}"
            while true; do
                cmd="squeue -u ${SERVER_USER} -j ${jobID}"
                job_status=$(ssh "${SERVER_USER}@${SERVER_ADDRESS}" ${cmd} 2>/dev/null)
                job_status=$?
