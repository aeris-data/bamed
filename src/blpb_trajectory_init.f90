...
PROGRAM BLPBTRAJ
    ...
    do ibal=1,nbal
        ...
        write(fout(ibal),'("launch_lat=",f15.10)') yb(ibal)/pi180
        write(fout(ibal),'("launch_lon=",f15.10)') xb(ibal)/pi180
        write(fout(ibal),'("launch_date=",i14.8)') start(1)
        write(fout(ibal),'("launch_time=",i14.6)') start(2)
        write(fout(ibal),'("density=",f18.3)') rhob(ibal)
        write(fout(ibal),*) '       time    longitude     latitude altitude  variable1   variable2'
    end do
    ! write(*,*)F    
    ...
    do while ((tps.lt.tf).and.(flight))
        ...
        do ibal=1,nbal
            ...
            if (status(ibal).eq.1) then
                ...
                blon=xb(ibal)*ipi180
                blat=yb(ibal)*ipi180
                write(fout(ibal),'(f12.2,x,f12.8,x,f12.8,x,f8.2,x,f10.6,x,f11.8)')&
