...
function main(){
    ...
    DATA_END_DATE=$(date -d "$(date -d "${END_DATE}" +%Y%m%d) + 1 day" +%Y%m%d)
...
EOF
    ...
    if [ -z ${SERVER_USER} ] || [ -z ${SERVER_ADDRESS} ] || [ -z ${SERVER_DATA_DIR} ]; then
        ...
        if (( ${NOT_TRANSFERRED_FILES} != 0 )); then 
            ...
            ssh "${SERVER_USER}@${SERVER_ADDRESS}" ${cmd}
            scp -o ServerAliveInterval=10 -o ServerAliveCountMax=5 -q "${SIMU_CONFIG_FILEPATH}" "${DST_PATH}/"
            scp -o ServerAliveInterval=10 -o ServerAliveCountMax=5 -q "${REMOTE_JOB_FILEPATH}" "${DST_PATH}/"
